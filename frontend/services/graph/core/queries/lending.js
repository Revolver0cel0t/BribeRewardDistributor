...
export const allMarketDayDatasQuery = gql`
  query marketDayDatas($first: Int = 1000, $date: Int = 0) {
    marketDayDatas(first: $first, orderBy: date, orderDirection: desc, where: { date_gte: $date }) {
      id
      date
      totalBorrows
      totalBorrowsUSD
...
export const liquidationDayDatasQuery = gql`
  query liquidationDayDatas($first: Int = 1000, $date: Int = 0, $seizedAddress: [String!]!) {
    liquidationDayDatas(
      first: $first
      orderBy: date
      orderDirection: desc
...
export const allLiquidationDayDatasQuery = gql`
  query liquidationDayDatas($first: Int = 1000, $date: Int = 0) {
    liquidationDayDatas(first: $first, orderBy: date, orderDirection: desc, where: { date_gte: $date }) {
      id
      date
      underlyingRepayAmountUSD
      underlyingCollateralSeizedAmountUSD
